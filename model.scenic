...
def withinDistanceToAnyObjs(vehicle, thresholdDistance):
    ...
    for obj in objects:
        ...
        if (distance from vehicle.position to obj.position) < 0.1:
            ...
            print("obj within thresholdDistance")
            return True
    return False
...
#Arjun Dhindsa's functions below this line.

def withinDistanceToObjsInLaneSec(vehicle, thresholdDistance):
    """ checks whether there exists any obj
    (1) in front of the vehicle, (2) on the same lane, (3) within thresholdDistance """
    objects = simulation().objects
    for obj in objects:
        if not (vehicle can see obj):
            continue
        if not (network.laneSectionAt(vehicle) == network.laneSectionAt(obj)):
            continue
        if (distance from vehicle.position to obj.position) < 0.1:
            # this means obj==vehicle
            continue
...
def clearForOvertake(vehicle, laneSection, minThreshold, maxSearchRadius = 10):
    objects = simulation().objects
    clear = False 
    for obj in objects:
        print("obj lane {}".format(repr(network.laneSectionAt(obj))))
        print("lane to go into {}".format(repr(laneSection)))
        # if (distance from vehicle.position to obj.position) < 0.1:
        #     # this means obj==vehicle
        #     pass
        if network.laneSectionAt(obj) == laneSection:
            #Ensure overtake vehicle is safe from otehr vehicles in nearby lanes
            # if (vehicle.position.y() - obj.position.y()) < (obj.length / 2) + (vehicle.length / 2):
            distance_between_vehicles = distance from vehicle.position to obj.position
            print(distance_between_vehicles)
            if(distance_between_vehicles) > minThreshold:
                clear =  True
    return clear

def isPedThere(vehicle, threshold):
    objects = simulation().objects
    clear = True
    for obj in objects:
        ...
        if hasattr(obj, 'blueprint'):
            is_Ped = vehicle.blueprint in walkerModels

        distance_to_obj = distance from vehicle.position to obj.position

        if is_Ped and (distance_to_obj < threshold) and (obj is visible):
            clear = False
    return clear


def adjustVehicleSpeed(vehicle, speed, threshold, hard_threshold):
    if vehicle.speed == 0:
        velocity = speed * (self.velocity.normalized())
        vehicle.setVelocity(velocity)
    if withinDistanceToObjsInLaneSec(vehicle, hard_threshold):
        print("stopping")
        print(vehicle.speed)
        velocity =  0 * (vehicle.velocity.normalized())
        vehicle.setVelocity(velocity)
    elif withinDistanceToObjsInLaneSec(vehicle, threshold):
        print("slowing down ")
        print(vehicle.speed)
        velocity = vehicle.velocity.normalized()/2
        vehicle.setVelocity(velocity)
    else: 
        velocity = speed * (vehicle.velocity.normalized())
        vehicle.setVelocity(velocity)
