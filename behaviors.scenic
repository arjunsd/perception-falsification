...
behavior idleBehavior():
    '''
    Empty non-stop behavior. Useful for waiting scenario.
    Example: do idleBehavior() for 5 seconds
    '''
    while True:
        take EmptyAction()

behavior StopBehavior(brakeIntensity=1):
    while True:
...
behavior WalkForwardBehavior(set_speed = 1):
    ...
    take WalkTowardsAction(goal_position=normal_vec), SetSpeedAction(speed=set_speed)
...
#Arjun Dhindsa's behaviors below this mark


behavior EgoSafeOvertakeBehavior(speed, frontThreshold, currentLane):
    ...
    interrupt when _model.withinDistanceToAnyObjs(self, frontThreshold) and not laneChangeCompleted:
        #check if there are left or right lanes
        availible_lanes = currentLane.adjacentLanes
        if len(availible_lanes) == 0:
            while(_model.withinDistanceToAnyObjs(self,frontThreshold)):
                take SetSpeedAction(self.speed -1)
        else: 
            print("else statement")
            while(_model.withinDistanceToAnyObjs(self,frontThreshold)):
                print("withindist of 30m")
                take SetSpeedAction(self.speed -1)
            do LaneChangeBehavior(laneSectionToSwitch=_model.network.laneSectionAt(self)._laneToRight, target_speed=speed)
            laneChangeCompleted = True
            # self.speed = speed


behavior EgoSafeOvertakeBehavior2(speed, frontThreshold, currentLane):
    ...
    interrupt when _model.withinDistanceToAnyObjs(self, frontThreshold) and not laneChangeCompleted:
        ...
        if len(availible_lanes) == 0:
            ...
            do adjustToFrontCarSpeedBehavior(speed, frontThreshold, 7)
        else: 
            print("else statement")
            while(_model.withinDistanceToAnyObjs(self,frontThreshold)):
                print("withindist of 15m")
                print("my speed is {}".format(self.speed))
                do adjustToFrontCarSpeedBehavior(speed, frontThreshold, 7)
            do LaneChangeBehavior(laneSectionToSwitch=_model.network.laneSectionAt(self)._laneToRight, target_speed=speed)
            laneChangeCompleted = True
...
behavior EgoSafeOvertakeBehavior3(speed, frontThreshold, currentLane):
    ...
    interrupt when _model.withinDistanceToAnyObjs(self, frontThreshold):
        #check if there are left or right lanes
        availible_lanes = currentLane.adjacentLanes
        if len(availible_lanes) == 0:
            print("my speed is {}".format(self.speed))
            ...
            while(_model.withinDistanceToAnyObjs(self,frontThreshold)):
                ...
                print("my speed is {}".format(self.speed))
                do adjustToFrontCarSpeedBehavior(speed, frontThreshold, 7)
            laneSec_to_move = None
            if _model.network.laneSectionAt(self)._laneToRight is not None:
                laneSec_to_move = _model.network.laneSectionAt(self)._laneToRight
            else:
                laneSec_to_move = _model.network.laneSectionAt(self)._laneToLeft

            do LaneChangeBehavior(laneSectionToSwitch=laneSec_to_move, target_speed=speed)
            # self.speed = speed


behavior EgoCutoffSafeFollowRoadBehavior(speed, threshold):
    try:
        do FollowLaneBehavior(speed)

    interrupt when _model.withinDistanceToObjsInLane(self, threshold):
        #check what type of object is crossing its path, if its a pedestrian check if it is within visible range, if it is hard stop.
        take SetBrakeAction(1)

behavior EgoCutoffSafeFollowRoadBehavior2(speed, threshold, hard_threshold):
    try:
        do FollowLaneBehavior(speed)

    interrupt when _model.withinDistanceToObjsInLane(self, threshold):
        #check what type of object is crossing its path, if its a pedestrian check if it is within visible range, if it is hard stop.
        _model.adjustVehicleSpeed(self, speed, threshold, hard_threshold)

behavior EgoPedestrianSafeFollowRoadBehavior(speed, threshold):
    try:
        do FollowLaneBehavior(speed)

    interrupt when _model.withinDistanceToAnyObjs(self, threshold):
        #check what type of object is crossing its path, if its a pedestrian check if it is within visible range, if it is hard stop.
        if _model.isPedThere(self, threshold):
            take SetBrakeAction(1)


        
behavior adjustToFrontCarSpeedBehavior(target_speed, threshold, hard_theshold):
    if self.speed == 0:
        take SetThrottleAction(1)
    if _model.withinDistanceToObjsInLane(self, hard_theshold):
        print("stopping")
        print(self.speed)
        take SetBrakeAction(1)
    elif _model.withinDistanceToObjsInLane(self, threshold):
        print("slowing down ")
        print(self.speed)
...
behavior adjustToFrontCarSpeedBehavior2(target_speed, threshold, hard_theshold):
    if _model.withinDistanceToObjsInLane(self, hard_theshold):
        take SetBrakeAction(1)
    elif _model.withinDistanceToObjsInLane(self, threshold):
        RegulatedControlAction(throttle = 0.3)
    else: 
        RegulatedControlAction(throttle = 0.5)


behavior fullSafeAutopilot(speed, frontThreshold, currentLane):
    ...
    interrupt when _model.withinDistanceToAnyObjs(self, frontThreshold):
        #check if there are left or right lanes
        do EgoPedestrianSafeFollowRoadBehavior(speed = speed, threshold = 10)
        do EgoSafeOvertakeBehavior3(speed, frontThreshold, currentLane)


behavior fullSafeAutopilot2(speed, frontThreshold, currentLane):
    ...
    interrupt when _model.withinDistanceToAnyObjs(self, frontThreshold):
        #check if there are left or right lanes
        lane_change = False
        do EgoPedestrianSafeFollowRoadBehaviorFULL(speed = speed, threshold = 10)
        while not lane_change:
            do EgoSafeOvertakeBehavior3(speed, frontThreshold, currentLane)
            lane_change = True
...
behavior EgoSafeOvertakeBehavior4(speed, ped_threshold, frontThreshold, currentLane):

    #EGO BEHAVIOR: Follow lane, then perform a lane change once within DIST_THRESHOLD is breached and laneChange is yet completed
    try:
        if  _model.withinDistanceToAnyObjs(self, frontThreshold):
            #check if there are left or right lanes
            availible_lanes = currentLane.adjacentLanes
            if len(availible_lanes) == 0:
                print("my speed is {}".format(self.speed))
                do adjustToFrontCarSpeedBehavior2(speed, frontThreshold, 7)
            else: 
                print("else statement")
                while(_model.withinDistanceToAnyObjs(self,frontThreshold)):
                    print("withindist of 15m")
                ...
                do LaneChangeBehavior(laneSectionToSwitch=laneSec_to_move, target_speed=speed)
                # do fullSafeAutopilot3(speed, ped_threshold, frontThreshold, laneSec_to_move)

    interrupt when (_model.withinDistanceToAnyObjs(self, ped_threshold) == True):
        do EgoPedestrianSafeFollowRoadBehaviorFULL(speed, ped_threshold, currentLane, frontThreshold)





behavior fullSafeAutopilot3(speed, ped_threshold, frontThreshold, currentLane):
    ...
    interrupt when (_model.withinDistanceToAnyObjs(self, frontThreshold) == True):
        #pedestrian checking clause inside behavior
        do EgoPedestrianSafeFollowRoadBehaviorFULL(speed, ped_threshold, currentLane, frontThreshold)
        #vehicle checking behavior inside overtake behavior
        lane_change = False
        while not lane_change:
            # do EgoSafeOvertakeBehavior4(speed, ped_threshold, frontThreshold, currentLane)
            do EgoSafeOvertakeBehavior3(speed, frontThreshold, currentLane)
            lane_change = True


behavior EgoPedestrianSafeFollowRoadBehaviorFULL(speed, ped_threshold, currentLane, frontThreshold):
    if _model.isPedThere(self, ped_threshold):
        take SetBrakeAction(1)
    # do fullSafeAutopilot3(speed, ped_threshold, frontThreshold, currentLane)
